 ...
 *}
	...
	{capture assign="actionUrl"}{url router=$smarty.const.ROUTE_COMPONENT component="plugins.generic.addFrontendElements.controllers.grid.AddCitationGridHandler" op="updateCitation" submissionId=$submissionId escape=false}{/capture}
	...
	<form class="pkp_form" id="addCitationForm" method="post" action="{$actionUrl}">
		...
		{fbvFormArea id="addCitationArea" class="border"}
			{fbvFormSection label="plugins.generic.addFrontendElements.addCitations.itemStyle" for="style"}
				{fbvElement type="text" id="style" value=$style maxlength="255" inline=true multilingual=false size=$fbvStyles.size.MEDIUM}
			{/fbvFormSection}
			{fbvFormSection label="plugins.generic.addFrontendElements.addCitations.itemCitation" for="citation"}
